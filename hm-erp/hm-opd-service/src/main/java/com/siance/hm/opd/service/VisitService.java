...
/** Port of the original opd-service visit module: creation + daily token numbering + status lifecycle. */
@Service
...
public class VisitService {
    ...
    @Transactional
    public VisitResponse create(CreateVisitRequest request, UUID hospitalId) {
    ...
    public VisitResponse getById(UUID id, UUID hospitalId) {
        return toResponse(find(id, hospitalId));
    }

    @Transactional(readOnly = true)
    public PagedResult<VisitResponse> list(PageQuery query, UUID hospitalId) {
        Page<Visit> page = visitRepository.findByHospitalId(hospitalId, query.toPageable("createdAt"));
        List<VisitResponse> data = page.getContent().stream().map(this::toResponse).toList();
        return PagedResult.fromZeroBasedPage(data, page.getTotalElements(), page.getNumber(), query.getLimit());
    }

    @Transactional(readOnly = true)
    public List<VisitResponse> forPatient(UUID patientId, UUID hospitalId) {
        return visitRepository.findByHospitalIdAndPatientId(hospitalId, patientId).stream()
                .map(this::toResponse).toList();
    }

    @Transactional
    public VisitResponse updateStatus(UUID id, VisitStatus status, UUID hospitalId) {
    ...
    private Visit find(UUID id, UUID hospitalId) {
        return visitRepository.findByIdAndHospitalId(id, hospitalId)
    ...
    /**
     * Simplified per-hospital daily counter (original resets per department
     * per day). Good enough for a single front desk; swap for a
     * department-scoped Redis INCR if you need per-department queues.
     */

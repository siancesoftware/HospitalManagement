...
public class UpiService {
    ...
    @Transactional
    public UpiPaymentResponse createIntent(CreateUpiPaymentRequest request, UUID collectedBy, UUID hospitalId) {
        ...
        String payeeVpa = request.getPayeeVpa() != null ? request.getPayeeVpa() : upiProperties.getDefaultPayeeVpa();
        String payeeName = request.getPayeeName() != null ? request.getPayeeName() : upiProperties.getDefaultPayeeName();
        String note = request.getNote() != null ? request.getNote() : request.getPurpose().name();

        String deepLink = buildUpiDeepLink(payeeVpa, payeeName, request.getAmount(), note, payment.getUpiTxnRef());
        String qrUrl = buildQrImageUrl(deepLink);

        return new UpiPaymentResponse(paymentService.toResponse(payment), deepLink, qrUrl);
    }
     ...
     */
    @Transactional
    public PaymentResponse confirm(UUID paymentId, ConfirmUpiPaymentRequest request, UUID hospitalId) {
        Payment payment = paymentService.find(paymentId, hospitalId);
        if (payment.getUpiTxnRef() == null || !payment.getUpiTxnRef().equals(request.getUpiTxnRef())) {
            throw new ConflictException("Transaction reference does not match this payment.");
        }
    ...
    private String buildUpiDeepLink(String payeeVpa, String payeeName, BigDecimal amount, String note, String txnRef) {
    ...
    private String buildQrImageUrl(String data) {
        return upiProperties.getQrImageBaseUrl() + "?size=300x300&data=" + encode(data);
    }

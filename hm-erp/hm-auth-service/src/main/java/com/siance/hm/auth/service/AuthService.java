...
/**
 * Port of the original {@code AuthService}. Two extra methods
 * ({@link #createUser} / {@link #verifyCredentials}) exist purely because of
 * the microservice split: they let hm-hospital-service (and future owners of
 * other principal types) delegate credential storage/verification here
 * instead of touching a {@code users} table directly, exactly as the
 * database-per-service rule in the implementation plan requires.
 */
@Service
...
public class AuthService {
    ...
    public UserSummary createUser(CreateUserRequest request) {
        ...
        List<String> roleCodes = (request.getRoleCodes() == null || request.getRoleCodes().isEmpty())
    ...
    @Transactional
    public UserSummary verifyCredentials(VerifyCredentialsRequest request) {
        User user = userRepository.findByEmailIgnoreCase(request.getEmail())
                .orElseThrow(() -> new UnauthorizedException("Invalid credentials."));
    ...
    private void registerFailedAttempt(User user) {
        ...
        if (user.getFailedAttempts() >= MAX_FAILED_ATTEMPTS) {
            user.setLockedUntil(OffsetDateTime.now().plus(LOCK_DURATION));
        }
    }

    private void assignRole(User user, String roleCode) {
        Role role = roleRepository.findByCode(roleCode)
                .orElseGet(() -> roleRepository.save(new Role(roleCode, roleCode, true)));
        userRoleRepository.save(new UserRole(user, role));
    }

    private TokenResponse issueTokens(User user) {
        List<String> roles = roleCodesOf(user);
        TokenClaims claims = TokenClaims.platformAdmin(user.getId(), user.getEmail(), roles);
        ...
        TokenPair pair = TokenPair.bearer(accessToken, refreshToken, jwtTokenProvider.getAccessExpirationSeconds());
        return TokenResponse.of(pair, toProfile(user, roles));
    }
    ...
    private UserProfileDto toProfile(User user) {
        return toProfile(user, roleCodesOf(user));
    }
    ...
    private UserProfileDto toProfile(User user, List<String> roles) {
        return new UserProfileDto(user.getId(), user.getFirstName(), user.getLastName(), user.getEmail(),
                roles, user.getStatus().name(), user.isMfaEnabled(), user.getLastLoginAt());
    }

    private UserSummary toSummary(User user) {
        return new UserSummary(user.getId(), user.getFirstName(), user.getLastName(), user.getEmail(),
                user.isActive(), roleCodesOf(user));
    }
}

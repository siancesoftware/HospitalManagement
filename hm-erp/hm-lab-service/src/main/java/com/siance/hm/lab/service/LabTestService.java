...
public class LabTestService {
    ...
    @Transactional
    public LabTestResponse create(LabTestRequest request, UUID hospitalId) {
    ...
    public List<LabTestResponse> search(String query, UUID hospitalId) {
        List<LabTest> tests = (query == null || query.isBlank())
                ? labTestRepository.findByHospitalIdAndActiveTrue(hospitalId)
                : labTestRepository.findByHospitalIdAndNameContainingIgnoreCase(hospitalId, query);
        return tests.stream().map(this::toResponse).toList();
    }

    @Transactional
    public LabTestResponse update(UUID id, LabTestRequest request, UUID hospitalId) {
        LabTest test = labTestRepository.findByIdAndHospitalId(id, hospitalId)
                .orElseThrow(() -> ResourceNotFoundException.of("LabTest", id));
        apply(request, test);
        return toResponse(labTestRepository.save(test));
    }

    private void apply(LabTestRequest request, LabTest test) {
    ...
    private LabTestResponse toResponse(LabTest t) {
        return new LabTestResponse(t.getId(), t.getTestCode(), t.getName(), t.getLoincCode(), t.getSection(),
                t.getSampleType(), t.getTatHours(), t.getPrice(), t.isActive());
    }
}

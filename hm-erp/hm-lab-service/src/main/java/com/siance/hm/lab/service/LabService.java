...
public class LabService {
    ...
    public List<LabResponse> list(UUID hospitalId) {
        return labRepository.findByHospitalId(hospitalId).stream().map(this::toResponse).toList();
    }
    ...
    private LabResponse toResponse(Lab l) {
        return new LabResponse(l.getId(), l.getName(), l.getLocation(), l.isActive(), l.getHospitalId());
    }
}
